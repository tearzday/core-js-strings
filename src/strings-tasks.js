...
function isPalindrome(str) {
  const replaceStr = str.replaceAll(/[\s!?,]/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce((result, item) =>
      result.length > item.length - 1 ? result : item
    );
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[A-Z]/gi,
    (letter) => alphabetRot13[alphabet.indexOf(letter)]
  );
}
